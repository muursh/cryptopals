...
def decrypt_cbc(orig, key, IV):
    result_blocks = []
    previous_block = IV
    for block in partblock(orig, len(IV)):
...
def detect_encrypt_mode(encrypted):
    verify, ciphertext = encrypted('x'*(16*3))
    ch = partblock(ciphertext, 16)
